...
| XEP-0004 | Data Forms | ✅ | **The substrate of four other rows and of none of its own.** `DataForm` is `internal static`: reading a form and its fields, writing one back, and the boolean of the specification, which is `0`/`1` on the wire and neither of those in C#. It carries XEP-0045 (the room configuration form, a voice request, a held nickname), XEP-0060 (node configuration, publish options, subscription options, the authorisation of a pending subscription), XEP-0030 together with XEP-0128 (the extended information) — and through that last one XEP-0115, where the forms go into the `ver` hash sorted by their `FORM_TYPE` and one without a valid `FORM_TYPE` stays out, which section 5.4 says expressly. There is no public surface because no caller of this library has cause to build a form by hand; the callers that need one build theirs. The row exists all the same: a table of XEP support that leaves out what the other rows stand on invites the same question a second time |
...
| XEP-0045 | Multi-User Chat | ⚠️ | **The visiting half, the moderator's and the owner's.** Entering a room with or without a password, being in it, following who else is, leaving — `JoinRoomAsync`, `LeaveRoomAsync`, `ChangeRoomNickAsync`, `SetRoomSubjectAsync`, `SendRoomMessageAsync`, and `CreateInstantRoomAsync` for the one step of the owner protocol without which a room a join created stays locked for everybody else. The status codes of section 15.6 are read, which is what tells a departure from a kick, a ban and a nickname change — four things that arrive as the same stanza. Beyond that: the configuration form (`FetchRoomConfigAsync`, `ConfigureRoomAsync`, `MakeRoomNonAnonymousAsync`), moderating (`KickFromRoomAsync`, `BanFromRoomAsync`, `SetRoomRoleAsync`), the affiliation lists (`RoomAffiliationsAsync`, `SetRoomAffiliationAsync`), taking a room down (`DestroyRoomAsync`), inviting and refusing (`InviteToRoomAsync`, `DeclineInvitationAsync`), asking for a voice and answering that (`RequestVoiceAsync`, `AnswerVoiceRequestAsync`), holding a nickname over XEP-0077 (`ReserveRoomNicknameAsync`, `RoomNicknameAsync`), and the private word to one occupant of section 7.5 (`SendRoomPrivateMessageAsync`, `XMPPMessage.IsRoomPrivate`). What is **not** here: the rooms do not survive a reconnect, because after one this client is in none. Checked against Prosody's and ejabberd's room services, see "A room is not a roster" below |
| XEP-0313 | Message Archive Management | ⚠️ | **The asking half**, for one's own archive and for a room's: `LastFromArchiveAsync`, `RoomHistoryAsync`, `QueryArchiveAsync` with the filters of XEP-0004 and the paging of XEP-0059. The shape is the whole difficulty — **the results do not come back in the answer**, they arrive beforehand as ordinary-looking messages, and a client that hands those on replays its own history as new arrivals every time somebody opens a conversation. A result without a `<delay/>` is refused rather than filed under today; an empty `<before/>` is the last page and leaving it out is the first, which are both valid queries and only one of them is what opening a conversation wants. A result is turned into an `XMPPMessage` by a construction of its own, and D136 found that construction a field behind the live branch: knowing nothing of rooms, it read a private word out of a personal archive as an ordinary chat and a retraction as an ordinary message. It takes a room predicate now. Not here: preferences (which conversations are archived), and the flipped-page order some servers offer |
...
| XEP-0156 | Discovering Alternative XMPP Connection Methods | ✅ | The HTTP path only, and only as far as it is safe: `host-meta` is loaded exclusively over HTTPS, and only `wss://` endpoints are adopted. BOSH (`xbosh`) is read and passed over — this client does not speak it. The DNS path via `_xmppconnect` is not missing; it has been removed from the XEP. **"Exclusively over HTTPS" is asked of the address the answer came from, not only of the one that was requested** (D139) — a redirect is where those two part company, and until then the rule was held by a .NET default that nothing here stated and no test covered. What is *not* checked, on purpose, is whether the `wss://` host belongs to the JID domain: putting the service elsewhere is what this XEP is for, and its own rule is one about the certificate |
...
| XEP-0308 | Last Message Correction | ✅ | Receiving: `XMPPMessage.ReplacesId` names the superseded message, `IsCorrection` the fact. Sending: `CorrectLastMessageAsync` corrects the last message **to the same recipient** (section 5) and becomes the last one itself, so a correction can be corrected. **In a room too, since D135**, which needed two things the chat half did not: the last message has to be remembered under the room's bare address instead of under a full JID (`LastSentKey`), and the correction has to go out as `groupchat`, because a `chat` to a bare room address reaches nobody. Which id to name in a room the XEP does not say — both Prosody and ejabberd accept the room's own, measured rather than assumed. The same-sender rule of section 5 is the one that bites: a correction under somebody else's name puts words in their mouth. In the console `/fix <text>`; announced in disco#info (D60) |
...
| XEP-0454 | OMEMO Media Sharing | ✅ | **Both halves since D121.** `Encrypt` draws a fresh key *and* a fresh IV for every file — there is deliberately no overload taking a nonce, because AES-GCM forgives nothing about a repeated one under the same key — and `ToAesGcm` writes `aesgcm://host/path#[iv][key]` in lower-case hex; `TryParse`, `ToHttps` and `Decrypt` read it back, tag checked. `UploadManager.UploadEncryptedAsync` puts the **ciphertext** on an XEP-0363 service under a random name and `application/octet-stream`: a file called `passport.png` says most of what the encryption was for. What is *not* hidden is the size, near enough — padding is a decision with a cost and not one a library takes unasked. IV of 12 bytes only; the older 16 byte form is refused with a reason rather than silently misread. **The encryption is against the storage, not against the conversation**: the key travels in the fragment to whoever gets the message, so whoever can read the message can read the file. Checked against pyca in both directions — what the reference wrote we read, and since D121 what we write the reference reads |
| XEP-0084 | User Avatar | ✅ | Over XEP-0163, and **two nodes rather than one is the whole design**: the metadata node carries a few bytes and is what every subscriber is pushed; the data node carries the picture and is only ever fetched, by id, by whoever does not have it yet. A person with three hundred contacts and a new photograph sends three hundred short notices. `PublishAvatarAsync` publishes the picture *first* — announce first and every subscriber asks for something that is not there — and does not announce at all when the picture did not go. `RemoveAvatarAsync` publishes an empty `<metadata/>`, because a node left alone goes on announcing the old picture to everybody who subscribes later. **Bytes are checked against the id they were fetched under**, not because that makes the picture trustworthy but because the id is what everything downstream caches by. Three kinds of announcement and not two: a picture, a removal, and one that says nothing — the third must not arrive, or a malformed stanza takes a face off the screen. Not here, deliberately: decoding (an avatar is a picture from whoever is in the roster) and following the `url` of an `<info/>` (a library that fetches an address a contact put in a stanza has handed that contact a request from this machine) |
...
| XEP-0424 | Message Retraction | ✅ | Taking something back, which is not a correction with an empty text: XEP-0308 says *what I wrote was this instead*, this says *forget that I wrote it*, and clients and archives are allowed to treat the two differently. Receiving: `XMPPMessage.RetractsId` names the withdrawn message, `IsRetraction` the fact. Sending: `RetractAsync` takes back a message this side has in hand, `RetractMessageAsync` one by name. The namespace is `urn:xmpp:message-retract:1` — since version 0.4.0 it no longer hangs on XEP-0422, so what goes out is a plain `<retract/>` beside a XEP-0428 fallback and a XEP-0334 `<store/>` hint, one element for each of the three kinds of reader it has. **In a room the room's own id**, which is the one thing this XEP says outright where XEP-0461 arrives at it from the other side and XEP-0308 says nothing — so `RetractableId` is the same expression as `ReplyableId`, deliberately: they are one question, and two expressions would answer it differently one day. Processed only from the sender of the message it names. In the console `/unsay` |
...
| Endpoint discovery (XEP-0156 / `host-meta`) | ✅ With no endpoint given, `https://<domain>/.well-known/host-meta.json` and then `.../host-meta` are read — over HTTPS, checked at the address the answer came from and not only at the one requested (D139); only `wss://` addresses are taken. With no find it stays at `wss://<domain>:5443/ws` |
...
all of it — 1290 tests, of which the platform decides how many get an answer:
...
at 1277 until it could ask what was said before it arrived, at 1307 until it
could send something that is not text, at 1322 until it could send something the
server cannot read and at 1333 until it had a face, and a figure nobody updates
